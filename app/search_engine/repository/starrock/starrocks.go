	...
	go func() {
		defer func() {
			if err := recover(); err != nil {
				zap.S().Errorf("NewDirectUpload2-消费出现错误 :%+v", err)
			}
		}()
...
func (d *DirectUpload) StreamUpload() (count int, err error) {
	...
	for i := 0; i < count; i++ {
		...
		if err != nil {
			zap.S().Errorf("WriteString Error")
		}
	}
	err = write.Flush()
	if err != nil {
		zap.S().Errorf("write.Flush() :%+v", err)
	}
	...
	if err != nil {
		zap.S().Errorf("stream load error :%+v", err)
	}
